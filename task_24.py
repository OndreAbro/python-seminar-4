# Задача 24: В фермерском хозяйстве в Карелии выращивают чернику. Она растёт на круглой грядке,
# причём кусты высажены только по окружности. Таким образом, у каждого куста есть ровно два соседних.
# Всего на грядке растёт N кустов.  Эти кусты обладают разной урожайностью, поэтому ко времени сбора
# на них выросло различное число ягод — на i-ом кусте выросло ai ягод. В этом фермерском хозяйстве
# внедрена система автоматического сбора черники. Эта система состоит из управляющего модуля и
# нескольких собирающих модулей. Собирающий модуль за один заход, находясь непосредственно перед некоторым кустом,
# собирает ягоды с этого куста и с двух соседних с ним. Напишите программу для нахождения максимального
# числа ягод, которое может собрать за один заход собирающий модуль, находясь перед некоторым кустом
# заданной во входном файле грядки.

from random import randint

def create_list(length, left, right):
    return [randint(left, right) for i in range(length)]

try:
    n = int(input('Введите количество кустов на грядке: '))
    left = int(input('Введите левую границу диапазона значений урожайности: '))
    right = int(input('Введите правую границу диапазона значений урожайности: '))

    # harvest_list = []
    # for i in range(n):
    #     harvest_list.append(int(input(f'{i}-й куст:')))

    harvest_list = create_list(n,left,right)
    print(f"Ягоды, собранные с грядки: {harvest_list}")
    harvest_list.extend(harvest_list[:2])
    result = 0
    for i in range(1, n + 1):
        if sum(harvest_list[i - 1:i + 2]) > result:
            result = sum(harvest_list[i - 1:i + 2])
            index_list = [i - 1, i, i + 1]
    print(f"Максимальное число собранных за раз ягод: {result}")

except ValueError:
    print("Некорректный ввод данных!")


